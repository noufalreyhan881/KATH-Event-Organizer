...
export default function EditPortfolioPage() {
  ...
  useEffect(() => {
    ...
    if (id) {
      setTimeout(() => {
        ...
        setExistingImages(['/path-to-project-1.jpg', '/path-to-project-2.jpg']);
        setIsLoading(false);
      }, 800);
    }
  }, [id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log("Updated Data:", { ...formData, existingImages });
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#f3f4f6] font-sans text-gray-900 flex items-center justify-center">
        <div className="text-[#a68a2d] font-bold animate-pulse">Loading Project Data...</div>
      </div>
    );
  }
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-8">
          <div className="max-w-4xl mx-auto">
            <div className="mb-6">
              <Link href="/admin/portofolios" className="inline-flex items-center text-gray-500 hover:text-[#a68a2d] transition-colors">
                <ArrowLeftIcon />
                <span className="ml-2 font-medium">Back to Portfolio List</span>
              </Link>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Project Name */}
                  <div>
                    <label htmlFor="title" className="block text-sm font-medium text-gray-700 mb-1">Project Name <span className="text-red-500">*</span></label>
                  ...
                  {/* Client */}
                  <div>
                    <label htmlFor="client" className="block text-sm font-medium text-gray-700 mb-1">Client Name <span className="text-red-500">*</span></label>
                    <input type="text" id="client" name="client" value={formData.client} onChange={handleChange} className={`block w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-[#a68a2d]/20 focus:border-[#a68a2d] transition-colors ${errors.client ? 'border-red-300 bg-red-50' : 'border-gray-200'}`} />
                    {errors.client && <p className="mt-1 text-sm text-red-500">{errors.client}</p>}
                  </div>

                  {/* Date */}
                  <div>
                    <label htmlFor="date" className="block text-sm font-medium text-gray-700 mb-1">Event Date <span className="text-red-500">*</span></label>
                    <input type="date" id="date" name="date" value={formData.date} onChange={handleChange} className={`block w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-[#a68a2d]/20 focus:border-[#a68a2d] transition-colors ${errors.date ? 'border-red-300 bg-red-50' : 'border-gray-200'}`} />
                  ...
                  <div>
                    ...
                    <input type="text" id="categories" name="categories" value={formData.categories} onChange={handleChange} className={`block w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-[#a68a2d]/20 focus:border-[#a68a2d] transition-colors ${errors.categories ? 'border-red-300 bg-red-50' : 'border-gray-200'}`} />
                ...
                <div>
                  ...
                  <textarea id="description" name="description" rows={4} value={formData.description} onChange={handleChange} className={`block w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-[#a68a2d]/20 focus:border-[#a68a2d] transition-colors ${errors.description ? 'border-red-300 bg-red-50' : 'border-gray-200'}`}></textarea>
                ...
                <div className="flex items-center justify-end gap-4 pt-4 border-t border-gray-100">
                  <Link href="/admin/portofolios" className="px-6 py-2.5 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors">Cancel</Link>
                  <button type="submit" disabled={isSubmitting} className={`flex items-center px-6 py-2.5 bg-[#a68a2d] text-white rounded-lg font-medium shadow-md hover:bg-[#8c7324] transition-all ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''}`}>
